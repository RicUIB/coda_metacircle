...
*3.- Mostraremos los reemplazos composicionales para las técnicas del `tool kit` de análisis de datos de microbioma.*

*4.- `CODA en acción`: ejemplo de datos de la microbiota humana.*

## 1. Diferencias entre trabajar con conteos absolutos y conteos normalizados 🤯
...
-   Si el tamaño de la muestra es constante, los conteos de los microorganismos no son variables independientes.
...
<font color="blue"> Los cocientes logarítmicos tienen la propiedad de que su espacio muestral son números reales, una gran ventaja para la aplicación de los métodos estadísticos estándar desarrollados para variables aleatorias reales.</font>

A menudo se utiliza la transformación log-ratio centrada (clr) introducida Aitchison (1986).

<center>![](images/clr.PNG)</center>

-   Los valores son invariantes con respecto a la escala; es decir, se espera obtener la misma proporción en una muestra con pocos recuentos de lecturas o en una muestra idéntica con muchos recuentos de lecturas, solo se ve afectada la precisión de la estimación de clr. <font color="red"> La normalización de los conteos es innecesaria</font>.

-   Los valores transformados clr pueden utilizarse como entradas para pruebas de hipótesis multivariantes mediante herramientas como MANOVA, regresión, etc.

![](images/ceros.PNG){width="45%"}

-   La media (geométrica) no puede determinarse para datos dispersos sin eliminar, sustituir o estimar los valores de recuento 0.

-   Existen diversos métodos para tratar los valores de recuento tanto estimaciones puntuales utilizando el paquete `zCompositions` de R, como una distribución de probabilidad utilizando `ALDEx2` disponible en Bioconductor.
...
Existen varias técnicas para analizar la correlación de los datos del microbioma que suelen ser matrices "sparce". Uno de ellos es:

-   [SparCC]{.highlight} En lugar de calcular todas las correlaciones posibles, utiliza la técnica de "shrinkage" para estimar solo las correlaciones más fuertes y confiables. Esto permite lidiar con matrices de datos de alta dimensionalidad.
