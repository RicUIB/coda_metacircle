# CODA en acción 😎: Ejemplo 1 (A,E,I)
...
devtools::install_github("tpq/propr",force=TRUE) # tpq/propr
```
...
library(ALDEx2) # Bioconductor package
library(kableExtra)
library(ggplot2)
library(easyCODA)
...
library(propr) # https://github.com/tpq/propr
library(ppclust)
library(factoextra)
library(cluster)
library(fclust)
...
#  reducimos el data set para hacer pruebas
DF.0=DF.0[c(1:25,101,100,111,118,102,113,155),]
...
Damos un vistazo a lo que nos perderemos si las quitamos: Sus frecuencias relativas dentro de sus muestras únicas, En este caso no lo hacemos cogemos todos (ya hemos selecciona muestras interantes) lo veremos en otros ejemplos.
